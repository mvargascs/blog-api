package com.example.blogrest.Mocks;
...
public class BlogMockedData {
    ...
    public Blog updateBlog(int id, String title, String content) {
        Blog blogToUpdate = blogs.stream().filter(blog -> blog.getId() == id).findAny().orElse(null);

        if (blogToUpdate != null) {
            int blogIndex = blogs.indexOf(blogToUpdate);
            blogToUpdate.setTitle(title);
            blogToUpdate.setContent(content);

            blogs.set(blogIndex, blogToUpdate);
        }
    ...
    public boolean delete(int id) {
        ...
        if (blogToUpdate != null) {
            blogs.remove(blogToUpdate);
        }

        return blogToUpdate != null;
    }
}
